...
export default function SkillCard({ name, logoUrl, level }: SkillCardProps) {
    ...
    return (
        <div className="group flex items-center gap-3 rounded-lg border border-white/10 bg-white/5 p-3 transition hover:border-violet-500/40 hover:bg-white/[0.07]">
            <img
                ...
                width={28}
                height={28}
                className={`h-7 w-7 rounded-sm object-contain ${shouldBeWhite ? 'brightness-0 invert' : ''
                    }`}
            />
            <div className="flex-1">
                <div className="font-medium text-white">{name}</div>
                <div className="mt-2 h-1.5 w-full rounded bg-white/10">
                    <div className="h-1.5 rounded bg-gradient-to-r from-fuchsia-500 to-violet-600 transition-all duration-500" style={{ width: `${level}%` }} />
                </div>
            </div>
        </div>
    )
